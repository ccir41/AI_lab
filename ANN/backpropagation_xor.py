...
class NeuralNetwork():
    def __init__(self):
        ...
        self.hidden_to_output_weight = np.random.uniform(size=(self.hiddenNeurons, self.outputNeurons))
        self.hidden_to_output_bias = np.random.uniform(size=(1, self.outputNeurons))

    def forward(self, input_sequence):
        self.hidden_layer_activation = np.dot(input_sequence, self.input_to_hidden_weight)
        self.hidden_layer_activation += self.input_to_hidden_bias
        self.hidden_layer_output = self.sigmoid(self.hidden_layer_activation)

        self.output_layer_activation = np.dot(self.hidden_layer_output, self.hidden_to_output_weight)
    ...
    def backward(self, input_sequence, output_sequence, predicted_output):
        ...
        self.hidden_layer_output_error = self.output_delta.dot(self.hidden_to_output_weight.T)
        self.hidden_delta = self.hidden_layer_output_error*self.sigmoidPrime(self.hidden_layer_output)

        self.input_to_hidden_weight += input_sequence.T.dot(self.hidden_delta) * self.learning_rate
        self.input_to_hidden_bias += np.sum(self.hidden_delta, axis=0, keepdims=True) * self.learning_rate
        self.hidden_to_output_weight += self.hidden_layer_output.T.dot(self.output_delta)*self.learning_rate
        self.hidden_to_output_bias += np.sum(self.output_delta, axis=0, keepdims=True) * self.learning_rate
